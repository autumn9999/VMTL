...
def experiment(config, num_task):
    ...
    for iter_num in range(1, num_iter + 1):
        ...
        train_loader = iter(dset_loaders["train"])
        data_list = train_loader.next()

        order = range(num_task)
        order_list = list(order)
        random.shuffle(order_list)

        for i in order:
            inputs = data_list[0][i].transpose(0,1).contiguous().view(-1, 4096)
            labels = data_list[1][i].unsqueeze(1).repeat(1, config["bs_number"]).transpose(0,1).contiguous().view(-1)
            related_inputs = data_list[2][i].transpose(0,1).contiguous().view(-1, num_task-1,4096).transpose(0,1)

            inputs = inputs.cuda()
            labels = labels.cuda()
            related_inputs = related_inputs.cuda()

            model.optimize_model(inputs, labels, i, related_inputs)
        ...
        if iter_num % test_interval == 0 or iter_num == 1:
            ...
            for i in range(num_task):
                ...
                file_out.write('Iter {:05d} Acc on Task {:d}: {:.3f}\n'.format(iter_num, i, epoch_acc_list[i]))
                file_out.flush()

            # print best valuation and save models
            if np.mean(epoch_acc_list) > best_acc:
                ...
                save_dict = {}
                save_dict["shared_encoder"] = model.shared_encoder.state_dict()

                if config["model_type"] == "vmtl_ac":
                    save_dict["shared_generator"] = model.shared_generator.state_dict()
                    save_dict["shared_generator_bias"] = model.shared_generator_bias.state_dict()
                    for i in range(len(model.save_center_feature)):
                        save_dict["save_center_feature" + str(i)] = model.save_center_feature[i]
                else:
                    for i in range(len(model.specific_w_list)):
                        save_dict["specific_w_list" + str(i)] = model.specific_w_list[i].state_dict()

                for i in range(len(model.optimizer_list)):
                    save_dict["gumbel_list" + str(i)] = model.gumbel_list[i].state_dict()

                save_dict["iter_num"] = model.iter_num
                torch.save(save_dict, log_name + "/best_model.pth.tar".format(iter_num))
            ...
            print('Iter {:05d}: {:.3f}, {:.3f}, {:.3f}, {:.3f}\n'.format(best_iter_num, best_list[0], best_list[1],best_list[2], best_list[3]))
            file_out.write('Now- val Acc on Iter {:05d}: {:.3f}\n'.format(iter_num, np.mean(epoch_acc_list)))
            file_out.write('Best val Acc on Iter {:05d}: {:.3f}\n'.format(best_iter_num, best_acc))
            file_out.write('Iter {:05d}: {:.3f}, {:.3f}, {:.3f}, {:.3f}\n'.format(best_iter_num, best_list[0], best_list[1],best_list[2], best_list[3]))
            file_out.flush()
